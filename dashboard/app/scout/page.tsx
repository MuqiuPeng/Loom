...
import type { ScoutCandidate, ScoutLead, ScoutStats } from "@/lib/types";

/** The two lenses want opposite signals out of the same search.
 *
...
const LENS_COPY: Record<Lens, { label: string; hint: string }> = {
  job: {
    label: "Job hunt",
...
function score(c: ScoutCandidate, lens: Lens): number {
  if (lens === "job") {
...
export default function ScoutPage() {
  ...
  const [lens, setLens] = useState<Lens>("freelance");
  ...
  const shown = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => score(b, lens) - score(a, lens));
  }, [candidates, lens]);
  ...
  const loadSaved = useCallback(async () => {
    try {
      const res = await api.scout.listLeads();
  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="mb-4">
        ...
        <p className="mt-2 text-xs text-gray-500 max-w-3xl">
          {LENS_COPY[lens].hint}
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <label className="block">
            ...
            <input
              ...
              placeholder={
                lens === "job" ? "software development company" : "cafe"
              }
      ...
      {stats && (
        <div className="mb-4 flex flex-wrap items-center gap-4 text-sm text-gray-500">
          ...
          <span className="text-gray-400">
            · {shown.length} in this lens
          </span>
        </div>
      )}

      {/* The market-research payload: what's wrong across the whole batch. */}
      {stats && lens === "freelance" && Object.keys(stats.by_finding).length > 0 && (
      ...
      {stats && shown.length === 0 && !loading && (
        <p className="text-sm text-gray-400 py-8 text-center">
          Nothing in this lens — try the other one, or widen the category or area.
        </p>
      )}
